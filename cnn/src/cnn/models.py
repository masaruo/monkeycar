...
class ModelConfig(BaseModel):
    ...
    image_size: tuple[int, int] = Field(
        default=(160, 120), description="[Width, Height]"
    )
    norm_divisor: float = Field(
        default=255.0, gt=0, description="Normalization divisor"
    )
    ...
    final_val_loss: float | None = Field(
        default=None, description="Final validation loss"
    )
